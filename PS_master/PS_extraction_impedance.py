...
from SPS_parameters import (
    circumference_sps,
    harmonic_number_sps,
    momentum_compaction_sps,
    momentum_sps,
    voltage_sps,
)

from PS_bunch_rotation_generation import (
    C40_77_prog,
    C40_78_prog,
    C40_phase_prog,
    C80_08_prog,
...
# Other imports
from plots.mpl import uber_imshow, uber_plot
...
h84_delay = 12550 #12550
h168_delay = 78850 #79000 #78850
h84_adiabatic_ramp = 65000  # 65000

print(
    "h168_delay - h84_delay - h84_adiabatic_ramp: ",
    h168_delay - h84_delay - h84_adiabatic_ramp,
)
...
circumference_ps = 2.0 * np.pi * 100.0  # Circumference [m]
gamma_transition_ps = 6.1  # Transition gamma
momentum_compaction_ps = 1.0 / gamma_transition_ps**2  # Momentum compaction

# RF parameters PS
n_rf_systems_ps = 2  # Number of rf systems
harmonic_numbers_ps = [84, 168]  # Harmonic numbers
length_step_h168 = 0.1 * (
    h168_delay - h84_delay - h84_adiabatic_ramp
)  # First Step time [us] (old value: 133)
amplitude_step_h168 = 0.0  # Lin. step amplitude [kV]
C40_77_prog_factor = (
    271.34 / 300
)  # Calibration factor C40-77 (old value: 284.700 / 300)
...
C80_89_prog_factor = (
    277.58 / 300
)  # Calibration factor C80-89 (old value: 0.) # 277.58/300
...
adjusted_exponent = ((2 * adjusted_full_bl / adjusted_4sig) ** 2.0 - 3) / 2.0 - 0.5
...
n_bins_ps = int(
    n_bins_per_bunch_ps * (bunch_spacing_buckets_ps * (harmonic_numbers_ps[0] - 1) + 1)
)
...
# MHFB
impedance_reduction_target_C40_MHFB = -20.0 * 0
impedance_reduction_target_C80_MHFB = -20.0 * 0
...
for idx_sim in range(number_iterations_for_extraction):
    # Ring
    ring_ps = Ring(circumference_ps, momentum_compaction_ps, momentum, particle_type)
    # print('extraction_turn:', extraction_turn)

    if extraction_turn == -1:
        time_shortening = (
            20e-6 + 50e-6 + 230.65e-6
        )  # PEX.SEJ - PAX.SBRH84 - 6500 us  #1500e-6 for filamentation #294e-6 for local optimum
        print("time shortening: ", time_shortening)
        n_turns_ps = int(round(time_shortening / ring_ps.t_rev[0]))
    else:
        n_turns_ps = extraction_turn + 1

    ring_ps = Ring(
        circumference_ps, momentum_compaction_ps, momentum, particle_type, n_turns_ps
    )
    ...
    C40_77_RF_program = 1e3 * C40_77_prog(
        ring_ps.cycle_time * 1e6, start_bunch_rotation_h84 * 1e6, C40_77_prog_factor
    )

    C40_78_RF_program = 1e3 * C40_78_prog(
        ring_ps.cycle_time * 1e6, start_bunch_rotation_h84 * 1e6, C40_78_prog_factor
    )

    rf_prog_h84 = C40_77_RF_program + C40_78_RF_program

    phi_prog_h84 = C40_phase_prog(
        ring_ps.cycle_time * 1e6,
        start_bunch_rotation_h84 * 1e6,
        intial_phase_h84,
        final_phase_h84,
    )

    C80_08_RF_program = 1e3 * C80_08_prog(
        ring_ps.cycle_time * 1e6,
        start_bunch_rotation_h84 * 1e6,
        length_step_h168,
        amplitude_step_h168,
    ...
    C80_88_RF_program = 1e3 * C80_88_prog(
        ring_ps.cycle_time * 1e6,
        start_bunch_rotation_h84 * 1e6,
        length_step_h168,
        amplitude_step_h168,
    ...
    C80_89_RF_program = 1e3 * C80_88_prog(
        ring_ps.cycle_time * 1e6,
        start_bunch_rotation_h84 * 1e6,
        length_step_h168,
        amplitude_step_h168,
    ...
    omega_prog_h168 = (
        np.diff(phi_prog_h168) / rf_params_ps.t_rev[0] + rf_params_ps.omega_rf[1, 0]
    )
    ...
    #                      cuts_unit='s')
    cut_opt = CutOptions(
        n_slices=n_bins_ps, cut_left=0, cut_right=ring_ps.t_rev[0], cuts_unit="s"
    )
    ...
    if idx_sim == 0:
        if C80_89_prog_factor > 0:
            ZFactor_C80_MHFB = 3 / 2.0
            RshFactor_C80_HOMs = 3 / 2.0
        else:
            ZFactor_C80_MHFB = 1.0
            RshFactor_C80_HOMs = 1.0

    # Loading scenario at flat top
    PS_loader = ImpedanceLoader(
        MODEL=model,
        f_rev=ring_ps.f_rev[0],
        ...
        freq_array=np.linspace(0, 5.2e9, int(1e7)),
    )  # machineParams.freqArray
    ...
    Z_over_n = np.sum(np.array(ImZ_over_f_list[:-1]) * ring_ps.f_rev[0]) * np.ones(
        ring_ps.n_turns + 1
    )

    print("Revolution Frequency: ", ring_ps.f_rev[0])
    # # 10 MHz Cavity
    # my_res_10 = Resonators(800, 10*ring_ps.f_rev[0], 8)

    # # 20 MHz Cavity (R/Q = 31.75)
    # my_res_20 = Resonators(146.05, 20*ring_ps.f_rev[0], 4.6)

    # # 40 MHz Cavity (R/Q = 33)
    # my_res_40 = Resonators(2310, 40*ring_ps.f_rev[0], 70)

    # # 80 MHz Cavity (R/Q = 56)
    # my_res_80 = Resonators(5600, 80*ring_ps.f_rev[0], 100)

    # PS_intensity_time = InducedVoltageTime(beam_ps, profile_ps, ResonatorsList)
    ...
    PS_intensity_freq = InducedVoltageFreq(
        beam_ps,
        profile_ps,
    ...
    PS_inductive = InductiveImpedance(
        beam_ps, profile_ps, Z_over_n, rf_params_ps, deriv_mode="gradient"
    )  # Z_over_n

    PS_longitudinal_intensity = TotalInducedVoltage(
        beam_ps, profile_ps, [PS_intensity_freq, PS_inductive]
    )

    if plot_impeadance:
        # plt.figure('Impedance')
        uber_plot(
            PS_intensity_freq.freq / 1e6,
        ...
        plt.ylim(
            (
                ...
                1.1
                * np.max(
                    np.abs(PS_intensity_freq.total_impedance * profile_ps.bin_size)
                )
                / 1e3,
            )
        )
    ...
    # RF tracker
    longitudinal_tracker_ps = RingAndRFTracker(
        rf_params_ps,
        beam_ps,
        interpolation=True,
        Profile=profile_ps,
    ...
    if plot_induced_voltage:
        ...
        time_array = np.linspace(
            0, ring_ps.t_rev[0], len(PS_longitudinal_intensity.induced_voltage)
        )
    ...
    gaussian_4sig_save[0, 0] = (
        gaussian_fit(profile_ps.bin_centers, profile_ps.n_macroparticles)[-1] * 4
    )
    ...
    if n_bunches == 1:
        plots = Plot(
            ...
            0,
            (n_bunches)
            * rf_params_ps.t_rf[0, 0],  # <----- dt limits of the phase space plot
            E_min,
            E_max,  # <---------------- dE limits of the phase space plot
            show_plots=False,
            separatrix_plot=True,
        )

    ring_sps = Ring(
        circumference_sps, momentum_compaction_sps, momentum_sps, particle_type
    )

    rf_params_sps = RFStation(ring_sps, harmonic_number_sps, voltage_sps, 0, 1)
    ## Tracking
    for turn in range(ring_ps.n_turns):
        ...
        if plot_save_turns:
            if turn % dt_plt == 0:  # or (turn>120 and turn<130):
                ...
                plt.tight_layout()
                plt.savefig(
                    results_folder + "/bunch_distribution_turn_" + str(turn) + ".png"
                )
    ...
    extraction_turn = np.where(
        ...
        == np.nanmin(np.nanmean(gaussian_4sig_save, axis=0))
    )[0][0]
    print("bunch length: ", np.nanmin(np.nanmean(gaussian_4sig_save, axis=0)))
...
if plot_voltage_program:
    ...
    plt.clf()
    plt.plot(
        ring_ps.cycle_time * 1e6, C40_77_RF_program * 1e-3, label="C40_77_RF_program"
    )
    plt.plot(
        ring_ps.cycle_time * 1e6, C40_78_RF_program * 1e-3, label="C40_78_RF_program"
    )
    plt.plot(
        ring_ps.cycle_time * 1e6, C80_08_RF_program * 1e-3, label="C80_08_RF_program"
    )
    plt.plot(
        ring_ps.cycle_time * 1e6, C80_88_RF_program * 1e-3, label="C80_88_RF_program"
    )
    plt.plot(
        ring_ps.cycle_time * 1e6, C80_89_RF_program * 1e-3, label="C80_89_RF_program"
    )
...
if plot_phase_program:
    ...
    plt.plot(
        ring_ps.cycle_time * 1e6, phi_prog_h168 * 180 / np.pi, label="phi_prog_h168"
    )
    plt.xlabel("Time [us]")
...
# kde = gaussian_kde(xy)
sample_size = (
    10000  # Adjust the sample size as needed as heat map takes a while to plot
)
...
x_grid, y_grid = np.mgrid[
    0 : (n_bunches) * rf_params_ps.t_rf[0, 0] / 1e-9 : 100j, y.min() : y.max() : 100j
]
...
# Saving results
np.savez(
    results_folder + "/bunch_lengths.npz",
    fwhm_save=fwhm_save,
...
np.savez(
    results_folder + "/bunch_distribution.npz", beam_dt=beam_ps.dt, beam_dE=beam_ps.dE
)

np.savez(
    results_folder + "/transmission.npz",
...
if plot_animation:
    ...
    anim = FuncAnimation(
        fig, animate, init_func=init, frames=len(data_dt), interval=dt_plt, blit=True
    )

    anim.save(results_folder + "/bunch_rotation.gif", writer="pillow", fps=3)
...
if plot_impeadance:
    ...
    plt.clf()
    plt.plot(
        impedParams.freqArray,
        np.abs(impedParams.impedance),
        "k",
        label="Total",
        alpha=0.3,
    )
    ...
    plt.clf()
    plt.plot(
        machineParams.timeArray[: len(impedParams.inducedVoltage)] * 1e6,
        machineParams.beamCurrent[: len(impedParams.inducedVoltage)]
        / np.max(machineParams.beamCurrent)
        * np.max(np.abs(impedParams.inducedVoltage / 1e3)),
    )
    plt.plot(
        impedParams.timeArray[: len(impedParams.inducedVoltage)] * 1e6,
        impedParams.inducedVoltage / 1e3,
    )
    ...
    plt.hlines(0, impedParams.freqArray[0] / 1e9, impedParams.freqArray[-1] / 1e9, "k")
