...
class Resonators(_ImpedanceObject):
    ...
    def _imped_calc_python(self, frequency_array):
        ...
        for i in range(0, self.n_resonators):
            ...
            self.impedance[self.frequency_array>0] += self.R_S[i] / (1 + 1j * self.Q[i]
                                                 * (self.frequency_array[self.frequency_array>0] / self.frequency_R[i] -
                                                  self.frequency_R[i] / self.frequency_array[self.frequency_array>0]))
