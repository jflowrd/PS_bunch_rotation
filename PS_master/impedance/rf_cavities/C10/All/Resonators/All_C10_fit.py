'''
Fitting the C10-11 cavity impedance with resonators
...
for harmonic in harmonic_list:
    ...
    for cavity_name in cavity_list:
        ...
        data_path = script_path+'/../../Individual/'+cavity_name+'/Measurements/'
        ...
        if hasattr(impedParams, 'freqArray'):
        ...
        impedParams.addImpedanceTable(freq_data,
                                      real_data,
                                      imag_data,
                                      freqArrayInterp=freqArrayInterp)
    ...
    # Setting the fitting parameters, first run is coarse with only one
    # resonator but on all the data
    fittedParameters = impedParams.fitResonators(r_start,
                                                 f_start,
                                                 Q_start,
                                                 RShuntScale=1e3,
    ...
    R_S_save = np.array(fittedParameters[0][sorted_freq], ndmin=2) / 10  # Single cavity in file
    ...
    np.savetxt(script_path+'/single_resonator_h'+str(harmonic)+'.txt',
               saved_matrix, header='Impedance of C10 cavities summed' +
               '\nTotal impedance divided by 10 to have "single" cavity impedance' +
               '\nSingle resonator fit for h=%d' % (harmonic) +
               '\nAuthor: A. Lasheen' +
               '\nf_r [Hz]\t\t\t\tR_s [Ohm]\t\t\t\tQ')
    ...
    QBound = [[0.5, 10], [0.5, 5], [0.5, 10]][indexloop]
    ...
    np.savetxt(script_path+'/multi_resonators_h'+str(harmonic)+'.txt',
               saved_matrix, header='Impedance of C10 cavities summed' +
               '\nTotal impedance divided by 10 to have "single" cavity impedance' +
               '\nMultiple resonators fit for h=%d' % (harmonic) +
               '\nAuthor: A. Lasheen' +
               '\nf_r [Hz]\t\t\t\tR_s [Ohm]\t\t\t\tQ')
