'''
Fitting the measured C40-77 cavity impedance with resonators + direct
feedback model
...
def load_model(filename, folder='./', freqArray=None):
    ...
    if freqArray is None:
        freqArray = np.linspace(0, 160e6, 100000)
...
if __name__ == '__main__':
    ...
    #     new_freq = np.linspace(freq_data[(freq_data>38e6)*(freq_data<42e6)][0],
    # freq_data[(freq_data>38e6)*(freq_data<42e6)][-1], 1000)
    ...
    plt.plot(n_bunches,
             all_cases[:, 1] / 1e3, 'o--',
             label='Fit')
    plt.axhline(600, label='Unloaded', color='orange')
    plt.axhline(330, label='Loaded', color='red')
    ...
    plt.plot(n_bunches,
             all_cases[:, 2], 'o--',
             label='Fit')
    plt.axhline(18200, label='Unloaded', color='orange')
    plt.axhline(10000, label='Loaded', color='red')
    plt.xlabel('n bunches')
    ...
    plt.plot(n_bunches,
             ...
             label='Fit')
    plt.axhline(33, label='Expected', color='orange')
    plt.xlabel('n bunches')
    ...
    plt.plot(n_bunches,
             all_cases[:, 4], 'o--',
             label='Fit')
    plt.axhline(43, label='Expected', color='orange')
    plt.xlabel('n bunches')
