...
def final_filter_calc(frequency_array, revolution_frequency, harmonic_list,
    ...
    for index_h in range(len(harmonic_list)):
        ...
        loss_factor = 1-np.abs(params[index_h*3+2])
        b = np.array([(1-loss_factor), 0.])
        a  = np.array([1., -loss_factor])

        peak_filter = freqz(b, a,
                            worN=frequency_array,
                            fs=2*frequency_array[-1])[1]
                                                        
        peak_filter = np.interp(frequency_array,
                                np.append(np.flip(-frequency_array[1:]), frequency_array)+harmonic_list[index_h]*revolution_frequency,
                                np.append(np.flip(np.conj(peak_filter[1:])), peak_filter))
...
frequency_array = np.linspace(0, 160e6, 1000000)
...
#                     [162, 172, 163, 171, 164, 170, 165, 169, 166, 167]+[168]+[168]+[168])
        ...
        harmonic_list = np.array([162, 172, 163, 171, 164, 170, 165, 169, 166, 167]+[168])
