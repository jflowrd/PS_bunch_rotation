...
def load_model(filename_80MHz, revolution_frequency,
    ...
    if isinstance(filename_80MHz, str):
        if 'C80-08' in filename_80MHz.split('/'):
            C80_08_enabled = True
        if 'C80-88' in filename_80MHz.split('/'):
            C80_88_enabled = True
        if 'C80-89' in filename_80MHz.split('/'):
            C80_89_enabled = True
    elif isinstance(filename_80MHz, list) or \
            isinstance(filename_80MHz, np.ndarray):
        for filename in filename_80MHz:
            if 'C80-08' in filename.split('/'):
                C80_08_enabled = True
            if 'C80-88' in filename.split('/'):
                C80_88_enabled = True
            if 'C80-89' in filename.split('/'):
                C80_89_enabled = True
    ...
    if isinstance(main_harmonic_FB, str):
        if 'C80-08' in main_harmonic_FB.split('/'):
            C80_08_central_h_FB = True
        if 'C80-88' in main_harmonic_FB.split('/'):
            C80_88_central_h_FB = True
        if 'C80-89' in main_harmonic_FB.split('/'):
        ...
        for main_harmonic_FB_elem in main_harmonic_FB:
            if 'C80-08' in main_harmonic_FB_elem.split('/'):
                C80_08_central_h_FB = True
            if 'C80-88' in main_harmonic_FB_elem.split('/'):
                C80_88_central_h_FB = True
            if 'C80-89' in main_harmonic_FB_elem.split('/'):
                C80_89_central_h_FB = True

    # C80-08
    if C80_08_enabled:
        loaded_C80_08 = np.loadtxt(
            script_path+'/../../C80-08/Resonators/single_resonator_b72.txt',
            skiprows=4)
    ...
    if C80_88_enabled:
        loaded_C80_88 = np.loadtxt(
            script_path+'/../../C80-88/Resonators/single_resonator_b72.txt',
            skiprows=4)
    ...
    if C80_89_enabled:
        loaded_C80_89 = np.loadtxt(
            script_path+'/../../C80-89/Resonators/single_resonator_b72.txt',
            skiprows=4)
...
def final_filter_calc(frequency_array, revolution_frequency, harmonic_list,
    ...
    for index_h in range(len(harmonic_list)):
        ...
        b,a = iirpeak(harmonic_list[index_h]*revolution_frequency,
                      params[index_h*3+2],
                      fs=2*frequency_array[-1])

        peak_filter = freqz(b, a,
                            worN=frequency_array,
                            fs=2*frequency_array[-1])[1]

        delay = params[index_h*3]
        gain = params[index_h*3+1]

        final_filter += gain*peak_filter*np.exp(
...
#     residue = residue_center
    ...
    max_all = np.nanmax(impedance_in_window_center)

    residue_center = max_all**2
...
def residue_function_delay_all(params, *args):

    (harmonic_list,
     revolution_frequency, C80_impedance, delay_ratio, gain_ratio,
     Q_ratio, fixed_gain, fixed_Q) = args

    filter_params = []
    for index_h in range(len(harmonic_list)):
        filter_params += [1/revolution_frequency+params[index_h]*delay_ratio]
        filter_params += [fixed_gain*gain_ratio]
        filter_params += [fixed_Q*Q_ratio]
    ...
    max_center = np.nanmax(impedance_ratio_dB)
    residue = max_center**2.
...
#         [162, 172, 163, 171, 164, 170, 165, 169, 166, 167])#+[168])
    harmonic_list_init = np.array([168]+[162, 163, 164, 165, 166, 167, 169, 170, 171, 172])
...
#          3*(len(harmonic_list_init)+2)))
    ...
    OL_Resonator = Resonators(Rs, fr, Q)
    OL_Resonator.imped_calc(frequency_array)
    ...
    loop_gain_dB = 42.6
    fb_gain_44dB = 10**(loop_gain_dB/20)/Rs
    delay_n_rf = 18
    delay = delay_n_rf/fr

    fb_gain_max = np.pi/2 * 1/(Rs/Q) * 1/(2*np.pi*fr*delay)

    impedance_FB = feedback(frequency_array, OL_Resonator.impedance, delay, fb_gain_44dB)

    impedance_selector = [impedance_FB]

#                           C80_08_Resonator.impedance,
        ...
        for index_red in range(len(impedance_reduction_target_range)):
            for index_bw in range(len(bandwidth_target_range)):
                ...
                for index_cavity in range(len(impedance_selector)):
                    ...
                    # Adjusting delay
                    fixed_gain = 1e-3 / gain_ratio
                    fixed_Q = 15e3 / Q_ratio

                    startingParams_delay = []
                    startingParams_delay += [0.1e-9 / delay_ratio]
                    for index_h in range(len(harmonic_list)-1):
#                         startingParams_delay += [
#                             (-index_h*13/10.+3)*1e-9 / delay_ratio]
                        startingParams_delay += [
                            (3*(1-index_h/6.5))*1e-9 / delay_ratio]
                    fittedParameters_delay = np.array(startingParams_delay)
                    print(fittedParameters_delay)
                    
#                     fittedParameters_delay = minimize(
#                             residue_function_delay_all,
...
#                                              fixed_Q]
#                     fittedParameters_gain_delay=startingParams_gain_Q
...
#                     harmonic_list = np.array(
#                         [162, 172, 163, 171, 164, 170, 165, 169, 166, 167]+[168]+[168]+[168])
                    fittedParameters=[2.096537264095431e-06, 0.001, 15000.0, 2.0996981328512486e-06, 0.001, 15000.0, 2.099439970690504e-06, 0.001, 15000.0, 2.099226581615426e-06, 0.001, 15000.0, 2.0989781933882936e-06, 0.001, 15000.0, 2.0986146025282935e-06, 0.001, 15000.0, 2.0979222659963242e-06, 0.001, 15000.0, 2.0950708318519007e-06, 0.001, 15000.0, 2.0943350427465586e-06, 0.001, 15000.0, 2.0939421152317854e-06, 0.001, 15000.0, 2.0936085247123574e-06, 0.001, 15000.0]
